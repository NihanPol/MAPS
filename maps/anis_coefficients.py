...
#
# [Claude / MAPS optimization] Vendored from enterprise.signals.anis_coefficients
# so that MAPS no longer depends on `enterprise` for anisotropy-basis
# construction. Every routine here uses only numpy, scipy.special and healpy
# (all already MAPS dependencies). The functions are copied verbatim from
# enterprise so their outputs are bit-for-bit identical to the previous
# enterprise-backed code; subsequent MAPS commits add native optimizations on
# top (e.g. building Gamma_lm directly in (nclm, npairs) form).
#
# Original authors: Rutger van Haasteren (June 2014), Stephen Taylor (Feb 2016).
